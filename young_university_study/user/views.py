...
from .models import (
    ...
    Permission,
    User,
    user_has_college_permission,
    user_has_league_permission,
)
from .serializers import (
    CollegeRankInRangeRequestSerializer,
    CollegeRankInRangeResponseSerializer,
    CollegeRanksResponseSerializer,
    CollegeRequestSerializer,
    CollegeSerializer,
    ...
    UserRankInRangeResponseSerializer,
    UserSerializer,
...
class UserViewSetPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        # 只有校级管理员可以删除
        return user_has_league_permission(
            request.user, obj.college_id, obj.league_branch_id
        )
...
class UserViewSet(
    ...
    def search(self, request):
        ...
        if not self.request.user.is_superuser:
            ...
            for p in self.request.user.permissions.all():
                ...
                elif p.permission_type == ContentType.objects.get_for_model(
                    LeagueBranch
                ):
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得当前学生的排名，以及学生总人数",
        responses={200: RankResponseSerializer()},
    )
    def rank(self, request):
        rank = (
            User.objects.filter(total_study__gt=request.user.total_study).count() + 1
        )  # 计算为前面有多少人，所以+1
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得团支部的所有学生的信息",
        query_serializer=LeagueBranchRequestSerializer,
        responses={200: UserSerializer(many=True)},
    )
    def ranks(self, request):
        serializer = LeagueBranchRequestSerializer(data=request.query_params)
        serializer.is_valid(raise_exception=True)
        ...
        queryset = (
            User.objects.filter(college=college_id, league_branch=league_branch_id)
            .select_related("college")
            .select_related("league_branch")
            .prefetch_related("permissions")
        )
    ...
    def ranks_in_range(self, request):
        serializer = UserRankInRangeRequestSerializer(data=request.query_params)
        serializer.is_valid(raise_exception=True)
        ...
        queryset = (
            ...
            .annotate(
                total_study_in_range=models.Count(
                    ...
                    distinct=True,
                    filter=models.Q(
                        recording__study_id__gte=study_min,
                        recording__study_id__lte=study_max,
                    ),
                )
            )
...
class CollegeViewSet(
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得此学院的排名以及学院总数, 要求为校级管理员或此学院的管理员",
    ...
    def rank(self, request):
        ...
        if not user_has_college_permission(
        ...
        # TODO 优化这个for
        for c in (
            College.objects.annotate(total_study=models.Sum("user__total_study"))
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得所有学院的信息，以及学院的学习总数，要求为校级管理员",
    ...
    def ranks(self, request):
        ...
        queryset = College.objects.annotate(
            total_study=models.Sum("user__total_study")
        ).order_by("-total_study")
    ...
    def ranks_in_range(self, request):
        ...
        queryset = College.objects.annotate(
            ...
            total_study_in_range=models.Count(
                ...
                distinct=True,
                filter=models.Q(
                    user__recording__study_id__gte=study_min,
                    user__recording__study_id__lte=study_max,
                ),
            ),
        ).order_by("-total_study_in_range")
...
class LeagueBranchViewSet(
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        if not user_has_college_permission(
            request.user, serializer.validated_data["college"].id
        ):
            return Response(status=status.HTTP_403_FORBIDDEN)
        self.perform_create(serializer)
        headers = self.get_success_headers(serializer.data)
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得此团支部的排名, 要求为此团支部的管理员以及上级管理员",
        query_serializer=LeagueBranchRequestSerializer,
        responses={200: RankResponseSerializer()},
    )
    def rank(self, request):
        ...
        leagues = (
            LeagueBranch.objects.filter(college=serializer.validated_data["college_id"])
            .annotate(total_study=models.Sum("user__total_study"))
            .order_by("-total_study")
        )
        # TODO 优化这个for
        for league in leagues.iterator():
            if league.id is serializer.validated_data["league_branch_id"]:
                break
            rank += 1
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得此学院所有团支部的信息，要求为学院管理员和校级管理员",
        query_serializer=CollegeRequestSerializer,
    ...
    def ranks(self, request):
        ...
        if college_id == -1:
            queryset = LeagueBranch.objects.annotate(
                total_study=models.Count("user__recording__id")
            ).order_by("-total_study")
        else:
            queryset = (
                LeagueBranch.objects.filter(college=college_id)
                .annotate(total_study=models.Count("user__recording__id"))
                .order_by("-total_study")
            )
    ...
    def rank_in_range(self, request):
        ...
        queryset = (
            ...
            .order_by("-total_study_in_range")
            .get(college=college_id, id=league_branch_id)
        )
    ...
    @action(methods=["GET"], detail=False)
    @swagger_auto_schema(
        operation_description="获得此学院所有团支部的信息，要求为学院管理员和校级管理员，加入limit",
    ...
    def ranks_in_range(self, request):
        ...
        if college_id != -1:
            queryset = queryset.filter(college=college_id)
...
def check_permission(user, permission_type, permission_id):
    if (
        ...
        return True
    if permission_type == ContentType.objects.get_for_model(
        LeagueBranch
    ) and user_has_college_permission(user, permission_id):
        return True
    return False
...
class PermissionViewSet(mixins.CreateModelMixin, mixins.DestroyModelMixin, BaseViewSet):
